...
const KEYS = {
    ...
    LOCK_COMMENT_WORKER: "lock:comment-worker",
    LOCK_COMMENT_COUNT_WORKER: "lock:comment-count-worker",
    LOCK_NOTIFICATION_WORKER: "lock:notification-worker",
    LOCK_NEW_VIDEO_WORKER: "lock:new-video-worker",
...
async function processViews() {
    ...
    `;
        ...
        for (let i = 0; i < dirtyIds.length; i += CHUNK_SIZE) {
            ...
            // 3. Batch Update DB via parameterized UNNEST (no SQL injection risk)
            if (updates.length > 0) {
...
async function handleHistoryBatch(messages: [string, string[]][]) {
    ...
    if (upsertValues.length > 0) {
        ...
        // 2. Commit Core History Records in chunks to avoid unbounded transactions
        const CHUNK_SIZE = 100;
        for (let i = 0; i < upsertValues.length; i += CHUNK_SIZE) {
            const chunk = upsertValues.slice(i, i + CHUNK_SIZE);
...
async function handleCommentCountBatch(messages: [string, string[]][]) {
    ...
    // 1. Update Videos (parameterized UNNEST — no SQL injection)
    if (videoUpdates.size > 0) {
        const vidIds = Array.from(videoUpdates.keys());
        const vidDeltas = vidIds.map((id) => videoUpdates.get(id)!);
        await prisma.$executeRaw`
            UPDATE videos
            SET "commentCount" = GREATEST(0, "commentCount" + u.delta)
            FROM UNNEST(${vidIds}::text[], ${vidDeltas}::int[]) AS u(id, delta)
            WHERE videos.id = u.id
        `;
    }

    // 2. Update Comments (parameterized UNNEST — no SQL injection)
    if (commentUpdates.size > 0) {
        const cmtIds = Array.from(commentUpdates.keys());
        const cmtDeltas = cmtIds.map((id) => commentUpdates.get(id)!);
        await prisma.$executeRaw`
            UPDATE comments
            SET "replyCount" = GREATEST(0, "replyCount" + u.delta)
            FROM UNNEST(${cmtIds}::text[], ${cmtDeltas}::int[]) AS u(id, delta)
...
async function handleEngagementBatch(messages: [string, string[]][]) {
    ...
    if (reactions.length > 0) {
        ...
        // B. Batch Update Counts (parameterized UNNEST — no SQL injection)
        const countUpdates: ReturnType<typeof prisma.$executeRaw>[] = [];

        const updates = Array.from(videoDeltas.entries())
            .filter(([, d]) => d.likeDelta !== 0 || d.dislikeDelta !== 0)
            .map(([id, d]) => ({ id, ...d }));

        if (updates.length > 0) {
            const vidIds = updates.map((u) => u.id);
            const likeDeltas = updates.map((u) => u.likeDelta);
            const dislikeDeltas = updates.map((u) => u.dislikeDelta);
            ...
            countUpdates.push(
                prisma.$executeRaw`
                    ...
                    FROM UNNEST(${vidIds}::text[], ${likeDeltas}::int[], ${dislikeDeltas}::int[])
                         AS u(id, ld, dd)
                    WHERE videos.id = u.id
                `,
            );
...
async function handleCommentEngagementBatch(messages: [string, string[]][]) {
    ...
    if (reactions.length > 0) {
        ...
        const cmtUpdates = Array.from(commentDeltas.entries())
            .filter(([, d]) => d.likeDelta !== 0 || d.dislikeDelta !== 0)
            .map(([id, d]) => ({ id, ...d }));

        if (cmtUpdates.length > 0) {
            const cmtIds = cmtUpdates.map((u) => u.id);
            const likeDeltas = cmtUpdates.map((u) => u.likeDelta);
            const dislikeDeltas = cmtUpdates.map((u) => u.dislikeDelta);
            ...
            countUpdates.push(
                prisma.$executeRaw`
                    ...
                    SET
                        "likeCount"    = GREATEST(0, "likeCount"    + u.ld),
                        "dislikeCount" = GREATEST(0, "dislikeCount" + u.dd)
                    FROM UNNEST(${cmtIds}::text[], ${likeDeltas}::int[], ${dislikeDeltas}::int[])
                         AS u(id, ld, dd)
                    WHERE comments.id = u.id
                `,
            );
...
async function processCommentEngagement() {
    ...
    try {
        ...
        await redis.del(KEYS.LOCK_COMMENT_WORKER);
    }
}

async function processCommentCounts() {
    const locked = await redis.set(
        KEYS.LOCK_COMMENT_COUNT_WORKER,
        "1",
        "EX",
        15,
        "NX",
    ...
    try {
        ...
        await redis.del(KEYS.LOCK_COMMENT_COUNT_WORKER);
    }
}
...
async function processNotifications() {
    ...
    try {
        ...
        await redis.del(KEYS.LOCK_NOTIFICATION_WORKER);
    }
}
...
async function processNewVideoNotifications() {
    ...
    try {
        ...
        await redis.del(KEYS.LOCK_NEW_VIDEO_WORKER);
    }
}
