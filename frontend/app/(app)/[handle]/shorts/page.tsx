...
export default function ChannelShortsPage({ params }: { params: Promise<{ handle: string }> }) {
    ...
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = trpc.feed.getChannelShorts.useInfiniteQuery(
        { channelId: channelId! },
        { 
            enabled: !!channelId,
    ...
    if (videos.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground mt-8">
                <IconDeviceMobile className="w-12 h-12 mb-4 opacity-20" />
                <h3 className="text-lg font-bold text-foreground">No shorts</h3>
                <p>This channel hasn&apos;t uploaded any Shorts.</p>
            </div>
        );
    }

    // Wrap the standard VideoGrid in an aspect-ratio override container
    // Shorts covers use a 9:16 aspect ratio!
    return (
        <div className="w-full pb-20">
            {/* Override the aspect-video class on descendants */}
            <style jsx global>{`
                .shorts-grid .aspect-video {
