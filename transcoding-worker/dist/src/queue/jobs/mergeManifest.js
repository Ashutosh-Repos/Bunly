...
export async function handleMergeManifest(job) {
    const { videoId, resolutions, metadata } = job.data;
    const childrenValues = await job.getChildrenValues();
    let thumbnailOptions = [];
    let previewSprite = undefined;
    let previewSpriteVtt = undefined;
    Object.values(childrenValues).forEach((val) => {
    ...
    if (currentVideo?.processingStatus === "READY") {
        console.warn(`⚠️ Video ${videoId} already READY, skipping duplicate update`);
    ...
    const finalThumbnailUrl = currentVideo?.thumbnailUrl || thumbnailOptions[0] || null;
    console.log(`[Pipeline] 💾 Updating Database for ${videoId} (Status: READY)...`);
    ...
    const { cacheVideoStatus, publishToVideoChannel, deleteAggregateTracker } = await import("../../lib/pubsub.js");
    ...
    try {
        ...
        console.warn(`[Pipeline] ⚠️ Failed to cleanup raw source for ${videoId}`, e);
    }
    return { masterKey, thumbnailOptions, previewSprite, previewSpriteVtt };
}
