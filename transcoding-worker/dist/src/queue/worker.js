...
export const setupWorker = () => {
    worker = new Worker(QUEUES.TRANSCODE, async (job) => {
    ...
    worker.on("failed", async (job, err) => {
        if (job && job.data && job.data.videoId) {
            ...
            if (job.attemptsMade >= maxAttempts) {
                console.warn(`💀 Job ${job.name} exhausted retries. Updating DB, cleaning S3 & temp dir for ${videoId}...`);
                try {
                    ...
                    await deleteS3Prefix(`processed/${videoId}/`).catch((e) => console.warn(`⚠️ S3 cleanup failed for processed/${videoId}:`, e));
                    await deleteS3Prefix(`raw-videos/${videoId}/`).catch((e) => console.warn(`⚠️ S3 raw cleanup failed for raw-videos/${videoId}:`, e));
    ...
    worker.on("progress", async (job, progress) => {
        if (job.data?.videoId) {
            ...
            const { updateAggregateProgress } = await import("../lib/pubsub.js");
            // Extract context from either static job data or dynamic progress payload
            const resName = job.data.resolution?.name || progress.resolution;
            const resCount = job.data.resCount || progress.resCount;
