 ...
 */
export async function cacheVideoStatus(videoId, statusData) {
 ...
 */
export async function publishToVideoChannel(videoId, eventData) {
 ...
 */
export async function updateAggregateProgress(videoId, jobName, rawPercent, metadata) {
    const trackerKey = `video:${videoId}:progress:tracker`;
    // Map Job names to tracker fields
    let field = "";
    if (jobName === "probe-and-split")
        field = "probe";
    else if (jobName === "generate-thumbnails")
        field = "thumb";
    else if (jobName === "generate-sprite")
        field = "sprite";
    else if (jobName === "merge-manifest")
        field = "merge";
    ...
    try {
        ...
        const totalProgress = await redis.eval(luaScript, 1, trackerKey, field, String(rawPercent), resCount);
        if (totalProgress >= 0) {
        ...
        console.error(`[PubSub] ⚠️ Aggregate progress failed for ${videoId}:`, e);
    }
}
/**
 * Deletes the aggregate tracker from Redis once the job is finished or aborted.
 */
export async function deleteAggregateTracker(videoId) {
    const trackerKey = `video:${videoId}:progress:tracker`;
