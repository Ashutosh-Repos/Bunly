 ...
 */
export async function ensureOriginalFile(videoId, s3Key) {
    ...
    if (fs.existsSync(lockFile)) {
        ...
        try {
            ...
            if (timeString) {
                ...
                if (!isNaN(lockTime) && Date.now() - lockTime > 120000) {
                    console.warn(`[InputCache] ⚠️ Lock file ${lockFile} timestamp is stale (>2m old). Suspect dead pod, taking over...`);
                    try {
                ...
                console.log(`[InputCache] ⏳ Waiting for active download (Lock active): ${videoId}`);
                try {
                    await waitForDownload(lockFile, config.downloadTimeoutMs);
                }
                catch (e) {
                    console.warn(`[InputCache] ⚠️ Download timeout reached. Taking over...`);
                    try {
                        fs.unlinkSync(lockFile);
                    }
                    catch { }
        ...
        if (!fs.existsSync(localPath)) {
            console.warn(`[InputCache] ⚠️ File not found after wait, retrying download`);
    ...
    if (existingDownload) {
        console.log(`[InputCache] ⏳ Waiting for in-process download: ${videoId}`);
 ...
 */
async function waitForDownload(lockFile, timeoutMs) {
 ...
 */
export function isOriginalCached(videoId) {
    const localPath = path.join(getTempDir(), videoId, "source");
    const lockFile = path.join(getTempDir(), videoId, ".downloading");
