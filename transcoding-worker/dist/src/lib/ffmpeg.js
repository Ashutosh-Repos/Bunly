...
export async function killAllFfmpeg() {
    console.log(`[FFmpeg] 🗡️ Killing ${activeCommands.size} active FFmpeg processes...`);
 ...
 */
export function generateThumbnails(inputPath, outputDir) {
    return new Promise((resolve, reject) => {
        const fileNames = [];
 ...
 */
export function transcodeResolution(inputPath, outputDir, resolution, onProgress, options) {
    return new Promise((resolve, reject) => {
        ...
        switch (hwAccel) {
            ...
            case "videotoolbox":
                ...
                console.log(`[FFmpeg] 🍎 Using Apple VideoToolbox acceleration`);
                break;
            case "vaapi":
        ...
        // Audio passthrough disabled - mandatory re-encoding for EBU R128 volume normalization
        const useAudioPassthrough = false;
        const cmd = ffmpeg(inputPath);
        trackCommand(cmd);
        const outputOptions = [
            ...
            "-pix_fmt yuv420p", // Crucial: ensure 8-bit standard pixel format, preventing crashes on iPhone HDR/10-bit uploads
        ...
        if (hwAccel === "none") {
            ...
            // NVENC specific quality mode (lower is better, 28 is standard)
            outputOptions.push("-cq:v 28");
            outputOptions.push("-b:v 0"); // Required for nvenc to strictly respect cq
        }
        else if (hwAccel === "videotoolbox") {
            // VideoToolbox quality mode (0-100, roughly 50-70 is standard)
            outputOptions.push("-q:v 60");
        }
        else if (hwAccel === "vaapi") {
            // VAAPI quantization parameter
            outputOptions.push("-qp 24");
        }
        cmd.outputOptions(outputOptions)
            ...
            .audioFilters(["loudnorm=I=-16:TP=-1.5:LRA=11"]); // EBU R128 standard loudness target
        // --- Audio Settings ---
...
export function generatePreviewSprite(inputPath, outputDir, duration, width = 160, height = 90) {
    return new Promise((resolve, reject) => {
        ...
        cmd.inputOptions(["-y"]) // Overwrite
            ...
            `pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2`, // Center pad any remaining transparent space (black boxes) to retain exactly 160x90 cell slot
            "tile=10x10",
            ...
            const columns = 10;
            const rows = 10;
            const totalFrames = Math.min(100, Math.ceil(duration / interval));
...
export function createMasterPlaylist(outputDir, variants) {
    return new Promise((resolve, reject) => {
        let content = "#EXTM3U\n#EXT-X-VERSION:3\n";
        variants.forEach((v) => {
