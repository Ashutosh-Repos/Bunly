...
const createRedisClient = (isBullMQ = false) => {
    ...
    console.log(
        `[Redis] Initializing ${isBullMQ ? "BullMQ" : "Cache"} client with: ${logUrl}`,
    );

    return new Redis(redisUrl, {
        lazyConnect: true,
        maxRetriesPerRequest: isBullMQ ? null : 3, // BullMQ requires null
        retryStrategy(times) {
            if (times > 3) {
                console.error(
                    `[Redis] ${isBullMQ ? "BullMQ" : "Cache"} Connection failed after 3 retries`,
                );
                return null;
            }
...
export function getRedisConnection() {
    ...
    const baseConfig = { maxRetriesPerRequest: null }; // Required by BullMQ Queue/QueueEvents
    ...
    try {
        ...
        return { ...baseConfig, host: "localhost", port: 6379 };
    }
}
